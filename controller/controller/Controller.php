...
public function listResa() {
    ...
    public function updateResa() {
        if (!Session::is_connected()) {
            ...
            $etatFact = $_POST['etatFact'];
            $controller = 'Reservation';
            $view = 'create';
            $pagetitle = 'Mise à jour reservation';
            require FILE::build_path(array("view", "view.php"));
        }
    }
    public function updatedResa() {
        if (!Session::is_connected()) {
            self::festivalConnect();
        } elseif (!Session::is_admin()) {
            $controller = 'Accueil';
            ...
            $edit = new ModelReservation(0,0, $_POST['commentaire'], $_POST['prix'],0,$_POST['etatFact']);
            if ($edit->updated($_POST['numResa']) == false) {
                $controller = 'Accueil';
                $view = 'listVide';
                $pagetitle = 'Erreur lors de la creation';
                require FILE::build_path(array("view", "view.php"));
